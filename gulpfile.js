...
gulp.task('data', function() {
  return gulp.src('app/data/**/*.json')
    .pipe(gulp.dest('dist/interactive/2019/12/year-in-review-2019/data'))
    .pipe(browserSync.reload({
      stream: true
    }))
    .pipe( notify({ message: "data has been moved to dist!"}) );
});
...
gulp.task('watch', ['browserSync', 'sass', 'scripts', 'copyJSLibraries', 'data', 'images', 'fileinclude', 'html'], function (){
...
gulp.task('default', function (callback) {
  runSequence(['clean:dist', 'sass', 'images', 'fileinclude', 'scripts', 'copyJSLibraries', 'data', 'useref', 'html', 'browserSync', 'watch'],
    callback
  )
})
